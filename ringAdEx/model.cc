...
void modelDefinition(ModelSpec &model)
{
    ...
    SimpleAdEx::ParamValues p(
        ...
        0.0         // 7 - adaption current spike increase [pA]
    );

    SimpleAdEx::VarValues ini(
        -60.0,      // 0 - membrane potential V [mV]
        0.0        // 1 - Iw adaption current [pA]
    );
    
    model.addNeuronPopulation<SimpleAdEx>("Pop1", 10, p, ini);
